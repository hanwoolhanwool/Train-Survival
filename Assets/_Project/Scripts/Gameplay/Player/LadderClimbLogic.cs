...
namespace Game.Gameplay.Player
{
    ...
    public static class LadderClimbLogic
    {
        ...
        /// <summary>
        /// 방금 떨어진 사다리에 다시 붙는 것을 막는 창이 아직 열려 있는가.
        ///
        /// <para><b>이 창이 없으면 꼭대기에서 왕복이 생긴다.</b> 올라서기가 몸을 갑판 안으로 옮겨도
        /// 볼륨 이탈 콜백은 다음 물리 갱신에 와서 한 프레임 늦는다. 그 사이 오르던 입력(W)이 그대로면
        /// 다시 붙고, 평면 보정이 몸을 사다리 앞으로 되돌리고, 다음 프레임에 또 꼭대기 판정이 난다.</para>
        ///
        /// <para>점프 탈출도 같다 — 밀어내는 속도는 한 프레임에 몇 cm뿐이라 볼륨 안에 그대로 있다.</para>
        /// </summary>
        public static bool IsReattachBlocked(float now, float blockedUntil)
        {
