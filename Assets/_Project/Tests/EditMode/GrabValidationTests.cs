...
{
    /// <summary>
    /// 호스트 그랩 검증 규칙 (슬라이스 스펙 §2.4 — 거부 사유 3종 + M5 5차 등급 게이트).
    /// 기존 자원·손잡이는 전부 요구 등급 1이므로 1단계 집게 기준 판정이 5차 이전과 같아야 한다 (회귀 고정).
    /// <b>축 개명(GrabWeight → RequiredHarpoonTier) 후에도 이 파일이 그대로 통과하는 것이 로직 무변경의 증거다</b>
    /// (집게 단계별 파지 계획 §6.3).
    /// </summary>
    public sealed class GrabValidationTests
    {
        ...
        private const int Required1 = 1;

        [Test]
        public void 정상_보고는_승인된다()
        {
            GrabVerdict verdict = GrabValidation.Validate(
                true, false, Vector3.zero, new Vector3(0f, 0f, 15f), MaxRange, Tolerance, Tier1, Required1);
        ...
        {
            GrabVerdict verdict = GrabValidation.Validate(
                false, false, Vector3.zero, Vector3.forward, MaxRange, Tolerance, Tier1, Required1);
        ...
        {
            GrabVerdict verdict = GrabValidation.Validate(
                true, true, Vector3.zero, Vector3.forward, MaxRange, Tolerance, Tier1, Required1);
        ...
        public void 사거리_상한_초과는_거부된다()
        {
            GrabVerdict verdict = GrabValidation.Validate(
                ...
                MaxRange, Tolerance, Tier1, Required1);
        ...
        [Test]
        public void 집게_등급이_요구_등급에_못_미치면_거부된다()
        {
            GrabVerdict verdict = GrabValidation.Validate(
                true, false, Vector3.zero, Vector3.forward, MaxRange, Tolerance,
                harpoonTier: 1, requiredTier: 2);

            Assert.That(verdict, Is.EqualTo(GrabVerdict.InsufficientTier));
        }

        [Test]
        public void 집게_등급이_요구_등급과_같으면_승인된다()
        {
            GrabVerdict verdict = GrabValidation.Validate(
                true, false, Vector3.zero, Vector3.forward, MaxRange, Tolerance,
                harpoonTier: 2, requiredTier: 2);

            Assert.That(verdict, Is.EqualTo(GrabVerdict.Approved));
        }

        [Test]
        public void 집게_등급이_요구_등급보다_높으면_승인된다()
        {
            GrabVerdict verdict = GrabValidation.Validate(
                true, false, Vector3.zero, Vector3.forward, MaxRange, Tolerance,
                harpoonTier: 3, requiredTier: 1);
        ...
        public void 등급_게이트는_사거리_판정보다_뒤에_온다()
        {
            ...
            GrabVerdict verdict = GrabValidation.Validate(
                true, false, Vector3.zero, new Vector3(0f, 0f, MaxRange + Tolerance + 0.1f),
                MaxRange, Tolerance, harpoonTier: 1, requiredTier: 3);

            Assert.That(verdict, Is.EqualTo(GrabVerdict.OutOfRange));
        }

        [Test]
        public void 등급_게이트_순수_규칙은_요구_등급_이상이면_참이다()
        {
