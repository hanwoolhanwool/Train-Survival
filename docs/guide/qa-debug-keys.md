...
| `F6` | 진단 | **성능 HUD** 토글 — 프레임 p50/p95/max · 스레드별 ms · 병목 · 드로우콜 · GC. **눈으로 보는 용도이고 회귀 판정에는 쓰지 않는다**(에디터 플레이는 포커스를 잃으면 프레임이 안 흐른다). 판정은 빌드 벤치 `-perfrun` 이 한다 | 로컬 | 화면 좌상단 |
