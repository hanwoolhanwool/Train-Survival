...
F2 열차 높이 │ F3 시간 배속 │ F8 연출 모드 · F9 구속 · F10 시점
 └ 편성·건축   └ 사이클        └────── 연출·플레이어 3연속 ──────┘
```

넘치는 두 키만 아래 행으로 내려간다 — `0`은 자원·피해의 네 번째, `.`은 몬스터·보스의 네 번째다.
**`0`과 `.`은 각자 자기 그룹 열(列)의 연장선**이라고 보면 된다.

키는 네 곳에 흩어져 있고, 각각 **끄는 스위치가 따로**다.

| 소유 컴포넌트 | 키 | 끄는 곳 | 배선된 씬 |
|---|---|---|---|
| `QaDebugHotkeys` (Train) | 숫자패드 12종 · F2 | `_enableQaKeys` (인스펙터) | Game · Game_ArtTest |
| `DayCycleController` | 숫자패드 1·2·3 · F3 | `_enableDebugPhaseKeys` (인스펙터) | Game · Game_ArtTest |
...
> **숫자패드 15키는 만석이다.** 새 QA 키가 필요하면 F 계열에 붙인다 — **F4·F5·F6·F7이 비어 있다.**
> (F1은 에디터에서 Unity 문서를 여는 키라 피한다.)

---

## 2. 편성·건축 — 숫자패드 `7` `8` `9` `+` · `F2`
...
지역 전환은 Day 단위로 일어나므로 지역 검증에는 `3`을 쓴다.
...
| 씬 | 쓸 수 있는 키 |
|---|---|
| `Game.unity` | 숫자패드 12종 + 사이클 4종(1·2·3·F3) + F9 · F10 |
