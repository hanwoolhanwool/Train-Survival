...
> 상태: 설계 확정 · 구현 완료 (**§4.1 재결합만 설계 확정·구현 대기**)
...
- **재결합(연결부 재생성)** — 슬롯까지 끌어당긴 칸을 수리 망치로 재연결.
  **설계 확정 2026-08-01 (§4.1) · 구현 대기.** 이번 증분은 슬롯에서 '붙잡힌 채 정지'까지만.
...
### 4.1 재결합 (설계 확정 2026-08-01 · 구현 대기)
...
- `CouplingState[index-1]`: `Broken = false`, `Health = MaxHealth × 0.5` —
...
- ~~재결합(연결부 재생성) 수단·비용~~ → **2026-08-01 확정 (§4.1)**. 구현 대기.
