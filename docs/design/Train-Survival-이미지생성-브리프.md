...
### K.5.1 사막 원경 4층 — **생성이 아니라 절차 메시로 세웠다** ✅ *(1차 완료 2026-08-30)*

사막 1차의 중경 6조각·원경 4종은 **AI 생성 없이** 프리미티브와 절차 생성 메시로 만들었다.
이유는 예산이 아니라 **거리**다 — 800 m에서 투과율이 24 %라 표면 디테일이 화면에 남지 않고,
남는 것은 **실루엣과 비율**뿐이다. 생성물을 감축해 넣어도 화면이 같다.

| 에셋 | 방식 | tris | 실치수 |
|---|---|---:|---|
| `Mesh_Env_Dune_A/B/C` | 절차 생성 하이트필드 7×5 | **48** each | 사구 능선 — 폭 96~132 m · 높이 19~40 m |
| `Mesh_Env_Mesa` | 절차 생성 육각 프리즘 + 탈루스 | **28** | 중경·원경 공용. 폭 : 높이 = **2~3 : 1** 로 눕혀야 메사가 된다 |
| 원경 지면판 | 큐브 1개 | **12** | ±1,200 m · y −0.05 |

> **큐브를 그대로 세우면 800 m에서 "먼 도시"로 읽힌다.** 캡처 3회를 돌려 찾은 것은
> 텍스처도 폴리곤도 아니고 **폭:높이 비율**이었다. 1.7 : 1이면 빌딩, 2~3 : 1이면 메사다.
> 형태 언어(레벨 계획 결정 ⑨)는 **비율까지 포함해야** 성립한다.

**아직 생성이 필요한 것** — ~~3차 유적군~~ → **0종** (아래 §K.5.3 참조).

### K.5.3 사막 유적군 — **생성을 쓰지 않는 것이 규격이었다** ✅ *(3차 완료 2026-08-30)*

계획은 유적군을 "생성이 필요한 마지막 대상"으로 남겨 뒀다 — 500 m에서 투과율 57 %라
표면이 화면에 남기 때문이다. 그런데 **결정 ⑩이 표면 기호를 금지한다**:

> 상형문자·파라오 얼굴을 넣으면 세계관의 *"우리 이전에도 뭔가 있었다"* 가
> *"지구의 과거"* 로 고정된다. **형태는 빌리되 정체는 비워 둔다.**

표면에 그릴 것이 없으면 **텍스처가 필요 없고**, 텍스처가 없으면 생성이 이길 여지가 없다.
그래서 유적군도 절차 메시로 끝났다 — 그리고 **UV가 아예 없으니 나중에 기호가 새어 들어올
경로도 없다.**

| 에셋 | 방식 | tris | 실치수 |
|---|---|---:|---|
| `Mesh_Env_Pyramid` | 절차 — 사면 + **침식돼 잘린 정상** + 밑면 y −0.06(묻힌 밑동) | **12** | 대 120 · 중 90 · 소 60 m |
| `Mesh_Env_Sphinx` | 절차 — 몸통·앞발·가슴·**얼굴 없는 머리 블록**·두건·꼬리 | **96** | 길이 70 · 높이 20 m |
| `Mesh_Env_Lizard` | 절차 — 몸통·머리·꼬리 2단·다리 4 | **96** | 0.35 m |
| `S_DesertMirage` | 셰이더 — 알파의 **접히는 자리**를 사인 둘로 흔든다 | — | 띠 3장 · 높이 9/14/19 m |

> **신기루는 아지랑이가 아니라 하늘색 띠다.** 실제 하위 신기루는 달궈진 지면 위 공기층이
> 하늘을 반사하는 것이라, 흔들려야 하는 것은 밝기가 아니라 **경계선**이다. 바닥에 눕히면
> 눈높이 5.2 m에서 두께가 **0.23°(2픽셀)** 이라 지평선에 세워야 보인다.

### K.5.4 모래폭풍 입자 — 생성 텍스처 1장 (64²) *(4차 완료 2026-08-30)*

| 에셋 | 방식 | 규격 |
|---|---|---|
| `T_VFX_SandGrain` | 절차 — 방사 그라디언트 알파 | 64² · 알갱이 한 알 |
| `M_VFX_Sandstorm` | URP Particles/Unlit · 색 `(0.95, 0.88, 0.70)` | **폭풍 fog보다 밝아야 한다** |

**입자는 3층이어야 한다.**

| 층 | 방출 | 크기 | 없으면 |
|---|---:|---|---|
| 알갱이 | 1,300/s | 0.13~0.38 | — |
| 장막 | 190/s | 2.2~5.2 | 알갱이만 남아 **눈발**로 보인다 |
| 지표 포행 | 380/s | 0.8~2.4 | 발밑이 안 흘러 **감속이 눈에 안 보인다** |

> **정면으로만 날아오면 "워프"가 된다.** 첫 배치는 −Z 46 m/s 단일 방향이라 화면 중앙에서
> 방사되는 광속 이동처럼 읽혔다. 옆바람 17~34 m/s를 섞고 늘임을 3.2 → 2.0으로 줄여
> **대각선으로 흐르게** 하고서야 바람이 됐다.
>
> **투명 셰이더는 안개를 직접 계산해야 한다.** `S_DesertMirage`가 fog 를 안 받아
> 폭풍(밀도 0.035) 중에 신기루만 흰 판때기로 남았다. 색만 섞으면 알파 블렌딩이라 밝기가
> 남으므로 **알파도 함께 접는다.**

### K.5.2 사막 근경 — **여기도 절차 메시로 끝났다** ✅ *(2차 완료 2026-08-30)*

§K.6 사막 행이 지정한 전용 신규분 6종 중 **생성으로 만든 것은 0종**이다.

| 계획 (§K.6) | as-built | tris |
|---|---|---:|
| 선인장 2 | `Mesh_Env_Cactus_A`(기둥선인장 — 몸통 + 팔 2) · `Mesh_Env_Cactus_B`(통선인장 군락 3) | **92 · 72** |
| 모래언덕 조각 2 | `Mesh_Env_Dune_A/B/C` — **1차 원경 메시 재사용** | 48 each |
| 난파 열차 잔해 1 (3,000 지정) | `Mesh_Env_TrainWreck` — 몸통 + 남은 지붕 + 찢긴 끝 + 갈비뼈 3 + 드러난 차륜 2 | **128** |
| 메사 조각 1 | `Mesh_Env_Mesa` — **1차 원경 메시 재사용** | 28 |
| 마른 관목 1 · 마른 나무 1 | 미제작 — 자갈·바위·유목(상자)으로 대체 | — |

> **왜 생성이 이기지 못했나.** 이 여섯은 전부 **형태가 단순한 것**이다. 기둥선인장은 원기둥 3개고,
> 화차 잔해는 상자 6개 + 원기둥 2개다. AI 생성물을 3,000 tris로 감축해도 24 m에서 보이는 실루엣은
> 같은데, 절차 메시는 **감축 단계 자체가 없다.** 지면 텍스처
> (`T_Env_Ground_Desert_BaseColor` 512² · 심리스 3옥타브 + 바람 잔물결)도 코드로 구웠다.
>
> 이 판단이 뒤집히는 지점은 **표면이 보일 때**다 — 3차 유적군이 그렇다.

### K.5.5 대초원 — **원경·근경·들소 전부 절차 메시** ✅ *(1~3차 완료 2026-08-30)*

§K.6 대초원 행이 지정한 전용 신규분 중 **생성으로 만든 것은 0종**이다. 사막(§K.5.1·§K.5.2) ·
북극이 세운 판단이 세 번째로 성립했다 — **표면이 화면에 안 남으면 절차 메시가 이긴다.**
...
**원경 3층** (§K.5.1 사막 4층과 같은 방식)

| 대상 | 메시 | tris |
|---|---|---:|
| 거대 풍차 | `Mesh_Env_WindmillTower`(팔각 절두체) + `Mesh_Env_WindmillRotor`(허브 + 날개 4장) + 빌트인 큐브 나셀 | **32 + 80 + 12 = 124** / 1기 |
| 억새 구릉 | `Mesh_Env_GrassKnoll` — 낮은 돔 | **30** |
| 성토 제방 | `Mesh_Env_Embankment` — 사다리꼴 둑 (근경 수로 둑과 **공용**) | **12** |
| 황금 물결 지면판 | 빌트인 큐브 ±1,200 m | 12 |

**들소** `Mesh_Env_Bison` — 앞이 높고 뒤가 낮은 실루엣 + 뿔. **208 tris**
(계획 상한 1,200의 1/6). 스탬피드 개체와 비적대 중경 생물이 **같은 메시**를 쓴다.

**지면 텍스처 1장** `T_Env_Ground_Grassland_BaseColor` (512², 절차 생성) —
§K 프롬프트(*"dry autumn prairie, golden grain laid flat in drifts…"*)를 값 노이즈로 구현했다.
...
| **대초원** ✅ *(as-built §K.5.5)* | 벼밭 클럼프 2 · 억새 1 · 홀로 선 나무 **2**(줄기·수관) · ~~수로 조각 1~~(제방 메시 재사용) · 곡물 저장고 폐허 1 · **부러진 날개 1** · **들소 1** — **전부 절차 메시 · 생성 0종** | 지평선까지 이어진 황금 물결 + 거대 풍차 군락 |
...
### O.6 몬스터 2종 — 별들린 물고기·바닷새 *(4·5차 신규)*

**물고기**는 물에서만 살고 물 밖 표적에게 **튀어올라** 상판 위를 친다(4차).
**바닷새**는 갑판 위 **y 34**에 떠 있다가 급강하로 내려와 치고 다시 올라간다(5차) —
손 무기(산탄총 20 m)가 닿는 천장이 y 23.5라, **내려온 3.79초 동안만 잡힌다.**

> **바닷새는 실루엣이 전부다.** 순항 중에는 **30 m 위**에 있어 형태가 점으로 읽히고, 밤에는
> 물이 하늘빛을 반사해 밝게 남으므로 **하늘의 것은 검은 실루엣**이 된다. 날개를 접은 급강하
> 자세로 만들어 **위에서 볼 때와 아래에서 볼 때 모두 각진 삼각형**으로 읽히게 한다.
...
| `Monster_SeaFlyer` | 날개폭 2.6 m | 900 | a stylized corrupted seabird, broad angular wings held in a swept dive pose, thick chunky body, hooked beak, crystal shards growing from its back and wing joints, pale bone-white and dusty lavender #C0BEE0 plumage, emissive violet #8E7BE8 in the eyes and shards |
...
#### O.7.1 남는 여유는 **파도 유보분**이다 ⚠ *(2026-08-29 결정)*

교각·랜드마크에 더 쓰고 싶어지지만 **쓰지 않는다.** 그 자리는 파도가 받는다.

| 항목 | tris |
|---|---:|
| 배경 예산 (아트 예산 §6) | 270,000 |
| 바다 타일 9장 × 13,400 | −120,600 |
| **파도에 쓸 수 있는 여유** | **149,400** |

**물 평면은 지금 600 × 600 m짜리 큐브 한 장(12 tris)이다.** 파도(버텍스 변위)를 넣는다는 것은
여기에 **격자를 까는 것**이고, 격자 간격이 그대로 예산이 된다.
...
| **3 m** | **80,000** | ✓ |
| **4 m** | **45,000** | ✓ 넉넉하다 |
| 근거리 집중 (120 m · 1.5 m) + 원거리 큐브 | 12,800 | ✓ 품질 대비 가장 싸다 |

> **교각을 1,200 → 2,000으로 올리면** 타일당 +1,600 → 9장에 **14,400**이 빠진다. 3 m 격자와
> 4 m 격자를 가르는 폭보다는 작지만 **근거리 집중 LOD 하나를 통째로 날리는 크기**다.
> 물속 시점의 교각보다 **수면 전체가 움직이는 것**이 화면에 훨씬 크게 걸린다.
>
> **진짜 위험은 tris가 아니라 오버드로다** — 미결정 ④(타일 9장 × 물 평면, 알파 블렌딩이 화면
> 전체를 덮는다)에 파도는 버텍스 셰이더 비용을 더한다. 착수 시 그쪽을 먼저 잰다.

### O.8 이 시트 밖 — 아직 없는 것

| 대상 | 현재 | 비고 |
|---|---|---|
| **파도** | 없음 (검토 중) | 물 평면이 **큐브 한 장(12 tris)**이라 수면이 완전히 평평하다. 셰이더 + 격자 메시 축 — **예산은 §O.7.1에 유보해 뒀다** |
| **급강하 예고** | 없음 (§13.5 ⓒ) | 바닷새는 위치만 복제한다. 보스 돌진은 국면을 복제해 예고하는데(`_syncedChargeState`) 이쪽은 없다 — **그림자·울음소리**가 있어야 피할 수 있다. VFX·오디오 축 |
