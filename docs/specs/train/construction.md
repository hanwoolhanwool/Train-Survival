...
> **짝 문서**: [상호작용 중재](../player/interaction-arbitration.md) — 상자·작업대처럼 **나란히 붙은
> 건축물 중 어느 것이 E키를 받는가**는 그쪽이 소유한다
...
## 11bis. 덮는 범위 ≠ 막는 범위 (천막 1~3차 as-built · 2026-08-31)

원래 이 시스템은 **발자국 사각형이 곧 점유**였다. 천막이 그 등식을 깬다 —
지붕은 덮되 막지 않기 때문이다. 지금은 <b>세 범위가 각각 다르다.</b>

| 축 | 소유 | 범위 | 비고 |
|---|---|---|---|
| **점유**(설치를 막는 셀) | `StructureOccupancy` | `Solid` = 발자국 전체 / `Corners` = 네 모서리 | 기본 `Solid` — 기존 8종 판정 무변경 |
| **효과**(체온) | `ShelterScope` | `Car` = 칸 어디서든 / `Footprint` = 발자국 안 | 기본 `Car` — 난방기 규약 유지 |
| **비용** | `_costPerCell` | 0 = 채마다 고정값 / >0 = 넓이 비례 | 천막은 0(기둥 넷 값) |

**겹침 판정 규칙** — `StructureGridLogic.OverlapsExisting`:

- 같은 모양끼리(`Solid↔Solid`, `Corners↔Corners`)는 **사각형 교차만으로 충돌**.
  천막 안에 천막을 세우지 않는 것이 후자다.
- 다른 모양끼리(`Corners↔Solid`)만 **셀 단위**로 따진다 — 천막 안에 난방기가 들어가고,
  이미 선 건축물 위로 천막을 덮을 수도 있다. 한 함수(`OccupiesCell`)가 양방향을 성립시킨다.

**가변 크기** — `_resizable`이 켜진 종류는 카탈로그 발자국이 최소값일 뿐이고, 실제 크기를
설치할 때 정한다(우클릭 2회: 시작 셀 → 끝 셀). `StructureEntry`는 `CarIndex`가 byte 하나라
**칸을 넘는 한 채가 없으므로**, 드래그 사각형을 칸별로 잘라 칸마다 한 채를 세운다
(`ResizablePlacementLogic.ResolveSpans`). 발자국은 이미 항목에 실려 복제되고 있었으므로
**복제 규약은 바뀌지 않았다.**

> **커서는 언제나 1칸으로 스냅한다** — 가변 크기에서 커서는 "기둥 하나가 갈 자리"다.
> 카탈로그 발자국으로 스냅하면 `TryWorldToPlacementCell`의 클램프가 그만큼 안쪽으로
> 물러나 **격자 마지막 열·행에 커서가 닿지 않는다**(칸을 끝까지 덮을 수 없다).
