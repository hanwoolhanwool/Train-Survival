...
### 5.1 표면 마찰 — 접지 가속 (북극 3차, 2026-08-30)

`PlayerMotor.ComputeHorizontalVelocity`는 원래 **접지면에서 목표 속도를 즉시 돌려줬다** —
마찰이 무한이라는 뜻이다. 여기에 접지 가속 인자를 더했다. **0이면 종전과 완전히 같다.**

| 표면 | 접지 가속 | 걷기(4.5) 제동 | 달리기(7) 제동 | 어디에 |
|---|---:|---:|---:|---|
| 기본 (숲·사막·바다·대초원·열차 갑판) | **0 = 무한** | 0 | 0 | 값을 주지 않는다 |
| 눈 덮인 유빙 | **12 m/s²** | 0.84 m | 2.04 m | 북극 타일 10종 루트 |
| **맨 얼음 (빙판)** | **3 m/s²** | 3.38 m | **8.17 m** | 북극 E·F 세그먼트의 얼음 판 10장 |

- 값은 **`WorldFrameSurface`**(컨베이어 표시자)에 얹혀 있다. 밟은 표면을 찾는 경로가 이미
  그것 하나뿐이고(`ProbeGround`), **열차 갑판·지붕은 월드 소속이 아니라 이 경로를 타지 않아
  자동으로 제외**된다 — 지붕 결빙이 "구조는 열어 두고 적용만 보류"가 되는 이유다.
- `GetComponentInParent`가 **가장 가까운 조상**을 쓰므로, 타일 루트에 기본값을 두고 특정 얼음 판에만
  다른 값을 얹을 수 있다.
- 공중에서는 이륙 당시의 값을 유지한다 — 착지 프레임에 값이 튀면 제동 거리가 갈린다.

### 5.2 침수 — `PlayerSubmersion` (북극 3차)

| 축 | 계약 | 값 |
|---|---|---|
| 이동속도 | `IMoveSpeedModifier` | ×0.7 (동상 ×0.8과 곱해져 **×0.56**) |
| 환경 온도 | **`IAmbientTemperatureOverride`** (신설) | −2 ℃ · **단열 무효** |

- **복제가 없다.** 발 높이와 지역 물면은 이미 모든 피어가 같은 값을 갖고 있어 각자 유도한다 —
  소유자는 이속을, 서버는 체온을 같은 계산에서 읽는다.
- **수영이 아니라 잠김 깊이를 본다.** 얼음 틈은 잠김 0.72 m(바닥 −2.3 · `skinWidth` 0.08)라
  수영 진입 1.0에 못 미쳐 **걷기가 유지**되는데, 그래도 처벌은 받아야 "빠졌다"가 성립한다.
- 진입 0.25 / 이탈 0.12 히스테리시스 — `SwimMotion`과 같은 이유다.
- **단열 무효가 없으면** 방한 풀셋(0.9)이 침수 처벌을 통째로 지운다.

> `IAmbientTemperatureOverride`는 `IMoveSpeedModifier`와 같은 규약이되 **곱하지 않고 고른다** —
> 온도는 곱셈이 성립하지 않는 물리량이라(−2 ℃ × 0.7이 무슨 뜻인가) 먼저 값을 낸 구현체가 이긴다.

### 5.3 얼음 턱 기어오르기 — `IceLedgeMantleLogic` (북극 3차)

점프 높이 1.2 m 가 얼음 두께 1.5 m 를 못 넘는다. 얕은 물 바닥(−2.3)에서 얼음 상면(0)까지는
**2.22 m**(skinWidth 포함)라 더 멀다 — 빠지면 **반드시** 기어오르는 동작을 거친다.

- **트리거는 점프 키다.** 새 키를 만들지 않은 이유는 그 자리에서 점프가 **이미 소용없기** 때문이다.
- **저작이 없다.** 사다리는 볼륨을 두고 붙지만 얼음 턱은 지형 어디에나 있다 —
  레이캐스트 두 번(앞으로 · 그 너머에서 아래로) + 캡슐 검사 한 번으로 찾는다.
  올려놓는 동작은 사다리의 `LadderClimbLogic.ComputeMantleMotion`을 그대로 쓴다.
- 최대 상승 **2.9 m** — 두 상황을 동시에 덮는다:
  **얕은 물** 발 −2.22 → 상승 2.22 m · **깊은 물** 부력이 잠김 1.0~1.2 m 에서 멈춰
  발 −2.5~−2.7 → 상승 **최대 2.7 m**. **깊은 쪽이 더 멀다** — 헤엄치는 사람은
  수면에 떠 있지 서 있지 않기 때문이다. 그래도 **갑판 3.566 m 에는 닿지 않는다**
  (물에서 갑판으로 곧장 기어오르면 승차 사다리가 있을 이유가 없어진다).
...
### 6.2 표현 축 — 시점 모드·파지·애니메이션 (2026-08-17~19)
