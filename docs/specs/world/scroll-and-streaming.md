...
| 항목 | 내용 |
|---|---|
| **게스트 견인 전환 순간이동** (2026-07-21 발견, 수정 방향 확정·구현 대기) | 그랩 확정 시 게스트는 `_isTowed` 수신 전까지 컨베이어 유도를 계속 적용해 전환 순간 대상이 서버 고정 위치로 되돌아가는 스냅 발생. 채택된 수정안 A(클라이언트 예측 고정)에 따라 `ResourceNode`에 로컬 명중 시 컨베이어 제외·현재 위치 고정, 거부 시 컨베이어 복귀를 추가 예정 — 원인 분석·수정안 전체는 [harpoon 스펙 §11](../harpoon/grapple-pipeline.md) 참조 |
