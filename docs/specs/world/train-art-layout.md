...
## 9. 캐빈 실내등 — `Train/Car_Locomotive/Locomotive_Art/CabLamp` (2026-08-23)

`Train_Locomotive_HQ_Open`은 캐빈이 뚫려 있는데 **내부에 광원이 없어** 안이 거의 검게 보였다.
문·창으로 들어오는 빛만으로는 벽·바닥이 안 읽힌다. 천장등 하나로 해결했다.

### 9.1 캐빈 실측 — 레이캐스트로 잰 값

임시 `MeshCollider`를 붙여 캐빈 중앙에서 6방향으로 쏜 결과다. **모델 바운즈가 아니라 내부 공간**이다.

| 면 | world | 비고 |
|---|---:|---|
| 바닥 | y **3.91** | 갑판 상면(3.57)보다 0.34 높다 |
| 천장 | y **7.24** | 내부 높이 3.33 m |
| 좌우 벽 | x **±2.36** | 폭 4.72 m |
| 앞벽 | z **13.72** | 보일러 쪽 |
| 뒤 | (뚫림) | 문 개구부 — z ≈ 7.1 |
...
```
Car_Locomotive (비균등 4.6 / 3.4 / 13.603)
└ Locomotive_Art   보정 홀더 · lossy 1 · world (0, 0.92, 2.30)
  └ CabLamp        world (0, 7.22, 10.4)   ← 여기에 매단다
    ├ Train_CabLamp   FBX 인스턴스 (localPos 0 · 회전·배율은 FBX 기본값)
    └ CabLight        Point · localPos (0, −0.18, 0)
```

- **반드시 `Locomotive_Art` 아래에** 둔다. `Car_Locomotive` 직속은 비균등 스케일이라 램프가 찌그러진다
- 모델 원점을 **천장 부착면**으로 잡아 반출했으므로 y를 천장(7.24)에 맞추면 그대로 매달린다
  (7.22 = 0.02 묻어 이음매를 감춘다)

### 9.3 조명 값 — 실측으로 정했다

| 항목 | 값 | 왜 |
|---|---|---|
| Type | Point | 갓이 사방으로 여는 형태다 |
| Intensity | **45** | 8은 천장만 밝고 방이 안 읽힌다 · 90은 낮 창밖과 대비가 뒤집힌다 |
| Range | **12** | 캐빈 대각선이 8 m — 역제곱 감쇠 때문에 여유가 필요하다 |
| Color | `(1, 0.89, 0.72)` | 백열 |
| Shadows | **None** | 열차가 계속 움직여 포인트 그림자는 매 프레임 6면 갱신이다 |

> **URP 감쇠를 얕보면 안 된다.** 캐빈 벽 머티리얼이 짙은 청회색이라 반사가 거의 없어,
> 일반적인 실내 값(8~15)으로는 천장 언저리만 밝고 바닥은 그대로 검다.

### 9.4 모델 — `Train_CabLamp`

Meshy 생성물 `Bronze Sky Lantern`(이름과 달리 **천장 부착형 실링 램프**)을 §반입 규약대로 정규화했다.

| 항목 | 값 |
|---|---|
| 소스 | blend의 `zz_src_Train_CabLamp` (원본은 `zz_src_` 로 물러남) |
| 정규화 | 최대 치수 1 · **원점 = 천장 부착면 중앙**(z 0 ~ −0.297) |
