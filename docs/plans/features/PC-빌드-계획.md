...
작성일: 2026-08-23 · 상태: **1차 완료 — 빌드 성공·기동 확인·main 푸시** (2026-08-23, `main=43661da`) ·
브랜치: `build/pc-standalone-1st`
...
>
> **이력**: 작업은 `build/pc-standalone-1st` 에서 했고, 처음에는 `main` 을 건드리지 않는 것이
> 전제였다. 1차가 끝난 뒤 **판단이 바뀌어 브랜치 전체를 `main` 에 fast-forward 로 올렸다**
> (`main = 43661da`). 산출물(`Builds/`)은 여전히 커밋 대상이 아니다.
