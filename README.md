...
| 문서 | 내용 |
|------|------|
| **[성능 측정 자동화](docs/tech/performance-measurement.md)** | 3개월간 폴리곤만 세고 밀리초는 못 쟀던 이유(`enableFrameTimingStats: 0`)를 찾아, 빌드를 스스로 주행시켜 재는 자동화를 세우고 **최적화 우선순위를 실측으로 뒤집은** 기록. **측정 도구가 거짓말한 세 번**을 잡아낸 과정 포함 |
