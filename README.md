...
| 시점 | 목표 | 주요 항목 |
|---|---|---|
| **2026.09** | **기반 정리** — 릴리스 차단 요소 0 | 본편 `Game.unity` 궤도 이식(레벨 0차) · **레벨 디자인 1차 완료**(저작 기반, 완료 기준 = 회귀 0) · NGO 세션 재시작 해결 · 세션 이탈 시 클라 잔존 · 보스 패턴 4인 미발동 · Steam 실계정 로비·초대·릴레이 검증(M6 완료 기준) · **대형 클래스 5건 리팩터링**(9일 — [근거·순서](docs/plans/features/리팩터링-조사-보고서.md)) |
...
| **2026.11** | **세계를 채운다** — 지역이 색이 아니라 지형으로 읽힌다 | M8 3차 렌더 실측(Frame Debugger·GPU RD) → 레벨 2차 숲 파일럿 10종 → 3차 중경·원경 → 4차 나머지 3지역 30종 · 오디오 인프라와 사운드 반입 · UI 정리 |
...
**9월이 이 순서인 이유** — 궤도 타일과 열차 높이가 아직 검증 씬에만 있다.
이걸 본편으로 먼저 옮기지 않으면 그 뒤에 만드는 지형·배치물의 높이를 **전부 다시 잡아야 한다**.
레벨 1차를 코드 축(회귀 0)으로 먼저 끝내 두는 것도 같은 이유다 — 아트를 붓기 전에 받을 그릇을 만든다.
...
| 파일 | 줄수 | 한 클래스가 지는 것 |
|---|---:|---|
| `Gameplay/Train/TrainState.cs` | 1,531 | 상태 보유 · 조회 · 손상 · 수리 · 건축 3종 · 재결합 · 이탈 시뮬 · 클라 보간 |
...
**계산이 뭉친 게 아니라 배선이 뭉쳤다.** 순수 로직은 이미 `*Math`/`*Logic` 35파일로 빠져 있고
테스트 선언 844개가 그 위에 서 있다. 그래서 "로직을 더 빼라"는 처방은 이 코드에 듣지 않는다 —
